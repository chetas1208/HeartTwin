// Agent-specific types for the orchestration trace UI
...
export const AGENT_STAGES: AgentStageInfo[] = [
  {
...
export function buildPipelineStages(agentStatuses: Record<string, string>): PipelineStage[] {
  ...
  const stageNums = [...new Set(AGENT_STAGES.map((a) => a.stage))].sort()

  for (const num of stageNums) {
    const stageAgents = AGENT_STAGES.filter((a) => a.stage === num)
    const statuses = stageAgents.map((a) => agentStatuses[a.name] || 'pending')
    ...
    else if (statuses.some((s) => s === 'running' || s === 'success')) status = 'running'

    stages.push({ stage: num, agents: stageAgents, status })
  }
